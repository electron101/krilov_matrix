...
float** treugArr (float **Arr, int n) 
{
	...
	for (int i=0; i<n; i++)
	{
		...
		for (int j=i+1; j<n; j++)
		{
			//Коэффициент, на который будем домножать строку
			float koeff = Arr[j][i]/Arr[i][i]; 
			for (int k=0; k<n+1; k++)
...
int main (int argc, const char * argv[])
{
	...
	// Считываем размерность массивов Ni x Nj
	fscanf(file, "%d", &ni); 
	fscanf(file, "%d", &nj);
	...
	// Создадим массив c0, который будет хранить 
	// текущее значение C(1), C(2), C(3), C(4)
	c0 = new float*[ni];
	for (int i=0; i<ni; i++) 
		c0[i] = new float[1];

	// Перейдем к нахождению C(1), C(2), C(3), C(4), C(n)
	for (int i=0; i<ni; i++)
	{
	...
	/*--------------------------------------*/
	/* Расчитаем собсенные значения матрицы	*/
	/*--------------------------------------*/

	// Запишем систему уравнений для нахождения P1, P2, P3, P4, Pn
	treugArr(mC, ni);
	P = new float[ni+1];
	P = gSolve(mC, 5);
	P[ni+1] = 1;
	...
	dispArr(P, ni, nj, "%5.2f ");

	// Запишем получившееся уравнение с P(лямбда)
	std::cout << "Характеристическое уравнение: \n";
	for(int i = ni; i > 0; i--)
	{
		std::cout << P[i] << "\t Л^4 - (" << P[3] <<") Л^3 - (" << P[2] << ") Л^2 - (" << P[1] << ") Л - (" << P[0] <<") = 0 \n";
	lambda = new float[ni];

	// Ищем значения P(лямбда) методом половинного деления     
	for (int k=4; k>0; k--)
	{
		// Будем решать полученное уравнение 
		...
		// Разделим уравнение на полученный корень, 
		// чтобы получить кубическое
		P = urDiv(P, a, k);
		...
		{ 
			...
			std::cout << P[4] << " Л^4 - (" << P[3] <<") Л^3 - (" <<
				P[2] << ") Л^2 - (" << P[1] << ") Л - (" << 
				P[0] <<") = 0 \n"; 
		}
	}

	// Аккуратно выведем полученные значения P(лямбда)
	std::cout << "------------------------------ \n";
	std::cout << "Полученные собственные значения: \n";
	for (int i=ni; i>0; i--)
		std::cout << "Л[" << ni-i+1 << "]: " << lambda[i-1] << ";\n";
	std::cout << "------------------------------ \n";
	return 0;
}
